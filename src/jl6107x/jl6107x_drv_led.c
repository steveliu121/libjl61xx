...
jl_ret_t jl6107x_led_enable_set(jl_device_t *device, jl_led_group_t group, jl_portmask_t portmask)
{
	jl_ret_t ret = JL_ERR_OK;
	...
	swc_port_mask = portmask;
    /*only support to set utp port's led*/
	FLIP_BITS(swc_port_mask, 0, (LED_LED0_DISABLE_OFFSET_CFG_LED0_DISABLE_WIDTH-1));

	if (group == LED_GROUP0) {
		REGISTER_READ(device, LED, LED0_DISABLE, val, INDEX_ZERO, INDEX_ZERO);
		val.BF.cfg_led0_disable = swc_port_mask;
		REGISTER_WRITE(device, LED, LED0_DISABLE, val, INDEX_ZERO, INDEX_ZERO);
	} else if (group == LED_GROUP1) {
		REGISTER_READ(device, LED, LED1_DISABLE, val, INDEX_ZERO, INDEX_ZERO);
		val.BF.cfg_led1_disable = swc_port_mask;
		REGISTER_WRITE(device, LED, LED1_DISABLE, val, INDEX_ZERO, INDEX_ZERO);
	} else if (group == LED_GROUP2) {
		REGISTER_READ(device, LED, LED2_DISABLE, val, INDEX_ZERO, INDEX_ZERO);
		val.BF.cfg_led2_disable = swc_port_mask;
		REGISTER_WRITE(device, LED, LED2_DISABLE, val, INDEX_ZERO, INDEX_ZERO);
	} else
		return JL_ERR_PARAM;
...
jl_ret_t jl6107x_led_enable_get(jl_device_t *device, jl_led_group_t group, jl_portmask_t *pportmask)
{
	jl_ret_t ret = JL_ERR_OK;
	jl_portmask_t swc_port_mask = 0;
	...
	FLIP_BITS(swc_port_mask, 0, (LED_LED0_DISABLE_OFFSET_CFG_LED0_DISABLE_WIDTH-1));
	*pportmask = swc_port_mask;

	//ret = jlxx_portmask_c2l(device, swc_port_mask,  pportmask);
	return ret;
}

jl_ret_t jl6107x_led_group_config_set(jl_device_t *device, jl_led_group_t group, jl_led_config_t config)
{
	jl_ret_t ret = JL_ERR_OK;
	jl_uint32 src_mask = 0;
	...
	if (group == LED_GROUP0) {
		REGISTER_READ(device, LED, LED0_MODE, val, INDEX_ZERO, INDEX_ZERO);
		val.BF.cfg_led0_src_mask = src_mask;
		REGISTER_WRITE(device, LED, LED0_MODE, val, INDEX_ZERO, INDEX_ZERO);
	} else if (group == LED_GROUP1) {
		REGISTER_READ(device, LED, LED1_MODE, val, INDEX_ZERO, INDEX_ZERO);
		val.BF.cfg_led1_src_mask = src_mask;
		REGISTER_WRITE(device, LED, LED1_MODE, val, INDEX_ZERO, INDEX_ZERO);
	} else if (group == LED_GROUP2) {
		REGISTER_READ(device, LED, LED2_MODE, val, INDEX_ZERO, INDEX_ZERO);
		val.BF.cfg_led2_src_mask = src_mask;
		REGISTER_WRITE(device, LED, LED2_MODE, val, INDEX_ZERO, INDEX_ZERO);
	} else
		return JL_ERR_PARAM;
...
jl_ret_t jl6107x_led_group_config_get(jl_device_t *device, jl_led_group_t group, jl_led_config_t *pconfig)
{
	jl_ret_t ret = JL_ERR_OK;
	jl_uint32 src_mask = 0;
...
jl_ret_t jl6107x_led_blink_rate_set(jl_device_t *device, jl_led_blink_rate_t blink_rate)
{
	jl_ret_t ret = JL_ERR_OK;
	...
	REGISTER_READ(device, LED, LED_BLINK_RATE, val, INDEX_ZERO, INDEX_ZERO);
	val.BF.cfg_led_off_time = blink_rate;
	val.BF.cfg_led_on_time = blink_rate;

	REGISTER_WRITE(device, LED, LED_BLINK_RATE, val, INDEX_ZERO, INDEX_ZERO);
	return ret;
}

jl_ret_t jl6107x_led_blink_rate_get(jl_device_t *device, jl_led_blink_rate_t *pblink_rate)
{
	jl_ret_t ret = JL_ERR_OK;
